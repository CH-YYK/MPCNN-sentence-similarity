...
class MPCNN(object):
    ...
    def horizon_A(self, input1, input2):    # feah
        ...
        def func(m1, m2):
            return tf.concat([self.cosine_sim(m1[:,i], m2[:,i]) for i in range(m1.shape[1])], axis=1)

        output = tf.concat([func(input1[i], input2[i]) for i in range(input1.shape[0])], axis=0)
        return output
    ...
    def vertical_B(self, input1, input2):
        ...
        def func(m1, m2):
            ...
            for i in range(m1.shape[0]):
                lis += [self.cosine_sim(m1[i, :, j], m2[i, :, j]) for j in range(m1.shape[-1])]
            return tf.concat(lis, axis=1)
        output = tf.concat([func(input1[i], input2[i]) for i in range(input1.shape[0])], axis=0)
        return output

if __name__ == '__main__':
    word_vector = np.ones((3,3))
    embedding_size = 3
    filter_sizes = 4
    num_filters = 4
    sequence_length = 4
